    	...
    	if (len != sizeof(msg_t)) return;
    		else
    		{
    			...
    			if(TOS_NODE_ID != 1) // I am a node 
    			{
					...
					{
						if(msg -> topic == 0) dbg("radio_rec", "Received SUBACK from PAN coordinator! Node %d subscribed to topic TEMPERATURE\n", TOS_NODE_ID - 1);
						else if(msg -> topic == 1) dbg("radio_rec", "Received SUBACK from PAN coordinator! Node %d subscribed to topic HUMIDITY\n", TOS_NODE_ID - 1);
						else if(msg -> topic == 2) dbg("radio_rec", "Received SUBACK from PAN coordinator! Node %d subscribed to topic LUMINOSITY\n", TOS_NODE_ID - 1);
					...
					{ 
						...
						else if(msg -> topic == 1) dbg("radio_send", "Node %d sending a PUB to topic HUMIDITY with payload: %d%\n", TOS_NODE_ID - 1, msg -> data);
						else if(msg -> topic == 2) dbg("radio_send", "Node %d sending a PUB to topic LUMINOSITY with payload: %d lumen\n", TOS_NODE_ID - 1, msg -> data);
    			...
    			if(TOS_NODE_ID == 1) // I am the PAN
    			{
    				...
    				{	
						...
						else if(msg -> topic == 2) dbg("radio_rec", "PAN -> received PUB from node: %d to topic LUMINOSITY with payload: %d lumen\n", msg -> sender - 1, msg->data);
						...
						msg -> data = buffer[0][3];
				 		...
				 		if(msg -> type != 0)
				 		{
							...
							close(sockfd);	
							sleep(16); // It is 16 because Thingspeak can refrash only every 15s
						}			
    	 			}
    			}
    		}
